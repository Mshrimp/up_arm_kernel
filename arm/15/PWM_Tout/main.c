...
int main(void)
{
	...
	*/
	...
	set_one(ICDISER2_CPU0, 5);			//使能某一个中断;
	set_nbit(ICDIPR17_CPU0, 8, 8, 140);	//设置中断优先级别;
	set_nbit(ICDIPTR17_CPU0, 8, 8, 1);	//设置中断目标cpu ;	
										//00000001 : cpu0
										//00000010 : cpu1
										//00000011 : cpu0 & cpu1
	...
	*/
	...
	u32 start = TIMER_HZ * 100;
	//pwm_tout0_init(start);
	pwm_timer0_init(start);
	delay(1000000);
	u32 end = pwm_timer0_get_tcnto0();
	printf("time = %dus\n", (start - end) * 10);	
...
void do_irq(void)
{
	...
	switch(tmp & 0x3ff) 
	{
		...
		case 69:
			...
			//pwm_timer0_set_tcntb0(TIMER_HZ);
			//pwm_timer0_set_tcmpb0(TIMER_HZ/n);
			//n++;
			pwm_timer0_clean_irq();
			break;
	}
}
